"""Train EfficientNet-B0 face embeddings with triplet loss."""
...
def main() -> None:
    ...
    model = FaceEmbeddingModel(architecture="efficientnet_b0", pretrained=True)
    ...
    if not trainable_parameters:
        raise RuntimeError(
            "No trainable parameters found. Use --train-backbone or keep the "
            "default last-block fine-tuning setup."
        )
...
def _freeze_backbone_except_last_block(model: FaceEmbeddingModel) -> None:
    """Freeze most EfficientNet layers and fine-tune the last feature block."""
    for parameter in model.parameters():
        parameter.requires_grad = False

    feature_blocks = getattr(model.feature_extractor, "features", None)
    if feature_blocks is None or len(feature_blocks) == 0:
        for parameter in model.parameters():
            parameter.requires_grad = True
        return

    for parameter in feature_blocks[-1].parameters():
        parameter.requires_grad = True
